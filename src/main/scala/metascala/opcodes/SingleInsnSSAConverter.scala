  ...
  def apply(insn: AbstractInsnNode,
    ...
    def invokeVirtual(insn: MethodInsnNode, indexed: Boolean) = {
      ...
      if (insn.owner == "java/lang/invoke/MethodHandle" &&
        ...
        append(Insn.InvokeInterface(target, Agg.from(args), sig))
      }
    }
    ...
    insn.getOpcode match{
      ...
      case GETFIELD  => refField((a, b, c) => Insn.GetField(top(nextFrame), top(frame), a, b, c))
      case PUTFIELD  => refField((a, b, c) => Insn.PutField(top(frame), top(frame, 1), a, b, c))
